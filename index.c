...
int index_load(Index *index) {
    index->count = 0; // Initialize empty
    FILE *f = fopen(INDEX_FILE, "r");
    if (!f) return 0; // If file missing, index is just empty

    char hex[HASH_HEX_SIZE + 1];
    // Read each line: mode hash mtime size path
    while (index->count < MAX_INDEX_ENTRIES &&
           fscanf(f, "%o %64s %llu %u %[^\n]\n",
...
int index_save(const Index *index) {
    // 1. Sort entries by path (Git requirement)
    Index sorted = *index;
    ...
    // 3. Ensure data is physically on disk
    fflush(f);
    fsync(fileno(f));
    fclose(f);

    // 4. Atomic rename over the old index
...
int index_add(Index *index, const char *path) {
    struct stat st;
    if (stat(path, &st) != 0) return -1; // Get metadata

    // 1. Read file and write as OBJ_BLOB
    ...
    // Use provided get_file_mode from tree.c if available, or st.st_mode
    entry->mode = (uint32_t)st.st_mode; 
    entry->hash = blob_id;
    entry->mtime_sec = (uint64_t)st.st_mtime;
    entry->size = (uint32_t)st.st_size;

    // 3. Save index atomically
    return index_save(index);
}
